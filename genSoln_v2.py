...
#################################################
# usage :python3 genSoln_v2 kt Lmax 
#        python3 genSoln_v2 1 600   soluton at tau 
#        python3 genSoln_v2 10 600   soluton at 10*tau 
#
tau = 1e-5 
kappa1= 2.3
kappa2= 2.5
...
Hlm = np.zeros( alm.shape, dtype=complex)   # homo soln
Vlm = np.zeros( alm.shape, dtype=complex)   # inhomo soln
Wlm = np.zeros( alm.shape, dtype=complex)   # full solution
np.random.seed(2022)
for Lm in range(0,Lmax+1):
   Vm = np.zeros((Lm+1), dtype=complex)  # inhomogeneous
   Wm = np.zeros((Lm+1), dtype=complex)  # full solution 
   lam = Lm*(Lm+1)
...
randfield = hp.alm2map(alms=Wlm,nside=Nside)   
print('min/max(full) = ', min(randfield), max(randfield))
map_type = 'Full_soln'
sv_fig = map_type + '_Lmax' + str(Lmax) + '_t_'+str(kt)+'tau_tau_1e_5.png'
plt.figure(1)
cm = cmbcmap()
ti =r'Full solution $u_L(t)$ at $t=%d\tau$, with ${\tau}=10^{-5}$ ' % kt + ', $L= %d$' %Lmax
ti = ''
hp.mollview(randfield, title=ti, cmap=cm, min=-5, max=5, xsize=1200, nest=False)
plt.title(ti)
plt.savefig(sv_fig,format='png',dpi=600)

Inhomo_randfield = hp.alm2map(alms=Vlm,nside=Nside)   
print('min/max(inhomo) = ', min(Inhomo_randfield), max(Inhomo_randfield))
map_type = 'Inhom_soln'
sv_fig = map_type + '_Lmax' + str(Lmax) + '_t_'+str(kt)+'tau_tau_1e_5.png'
plt.figure(2)
cm = cmbcmap()
ti =r'Inhomogeneous solution $u_L(t)$ at $t=%d\tau$, with ${\tau}=10^{-5}$ ' % kt + ', $L= %d$' %Lmax
ti = ''
hp.mollview(Inhomo_randfield, title=ti, cmap=cm, min=-5, max=5, xsize=1200, nest=False)
plt.title(ti)
plt.savefig(sv_fig,format='png',dpi=600)

homo_randfield = hp.alm2map(alms=Hlm,nside=Nside)   
print('min/max(homo) = ', min(homo_randfield), max(homo_randfield))
map_type = 'Homo_soln'
sv_fig = map_type + '_Lmax' + str(Lmax) + '_t_'+str(kt)+'tau_tau_1e_5.png'
plt.figure(3)
cm = cmbcmap()
ti =r'Homogeneous solution $u_L(t)$ at $t=%d\tau$, with ${\tau}=10^{-5}$ ' % kt + ', $L= %d$' %Lmax
ti = ''
hp.mollview(homo_randfield, title=ti, cmap=cm, min=-5, max=5, xsize=1200, nest=False)
plt.title(ti)
plt.savefig(sv_fig,format='png',dpi=600)
